        ...
        while capture.isOpened():
            ...
            if len(list_bboxs) > 0:
                for item_bbox in list_bboxs:
                    ...
                    center_x = (x1 + x2) / 2
                    center_y = (y1 + y2) / 2
                    pos[track_id] = (center_x, center_y)
                    if last_pos.get(track_id) is not None:
                        dis_pix = last_pos[track_id][1] - pos[track_id][1]
                        speed[track_id] = self.REAL_WIDTH(dis_pix) * self.FPS * 3.6
                        cv2.putText(output_image_frame, str(speed[track_id])[0:4], (x1, y1 + 20),
            ...
            yield output_image_frame, up_count, down_count
